...
async function getStadiumsWithCourtsAndTimes(stadiumId = null) {
  try {
    let stadiumQuery = DB.from('add_stadium')
    ...
    const { data: courts, error: courtError } = await DB.from('add_court')
    ...
    const { data: courtTimes, error: timeError } = await DB.from('court_time')
